...
export class PlaneBuilder extends BaseScriptComponent {
    ...
    private updateObjectParameters(isActive: boolean): void {
        if (isActive) {
            ...
            this.bone.getComponent(Interactable.getTypeName()).enabled = false
            this.bone.getComponent(InteractableManipulation.getTypeName()).enabled = false

        } else {
            print("Инструмент неактивен. Возвращаем исходные параметры.");

            this.matToolMarker.mainPass.isActive = 0
            this.bone.getComponent(Interactable.getTypeName()).enabled = true
            this.bone.getComponent(InteractableManipulation.getTypeName()).enabled = true
        }
    }
