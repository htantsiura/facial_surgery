    ...
    var PlaneBuilder = _classThis = class extends _classSuper {
        ...
        updateObjectParameters(isActive) {
            if (isActive) {
                ...
                this.bone.getComponent(Interactable_1.Interactable.getTypeName()).enabled = false;
                this.bone.getComponent(InteractableManipulation_1.InteractableManipulation.getTypeName()).enabled = false;
            }
            else {
                print("Инструмент неактивен. Возвращаем исходные параметры.");
                this.matToolMarker.mainPass.isActive = 0;
                this.bone.getComponent(Interactable_1.Interactable.getTypeName()).enabled = true;
                this.bone.getComponent(InteractableManipulation_1.InteractableManipulation.getTypeName()).enabled = true;
            }
        }
    };
