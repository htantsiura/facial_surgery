    ...
    onAwake() {
        ...
        this.createEvent("OnStartEvent").bind(() => {
            this.sliderX.onValueChange.add((value) => {
                let newPos = new vec3(this._teethCurrentPos.x + value / 20 - 0.5/20, this._teethCurrentPos.y, this._teethCurrentPos.z )
                mandibuleTrf.setLocalPosition(newPos)
                this.textX.text = "Position by X: " + (value / 20 - 0.5/20).toString
                print(value)
            })
