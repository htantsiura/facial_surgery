    ...
    onAwake() {
        ...
        this.createEvent("OnStartEvent").bind(() => {
            this.sliderX.onValueChange.add((value) => {
                let newPos = new vec3(this._teethCurrentPos.x + (value - 0.5) / 20, this._teethCurrentPos.y, this._teethCurrentPos.z )
                mandibuleTrf.setLocalPosition(newPos)
                let delta = (value - 0.5) / 20
                this.textX.text = "Position by X: " + delta.toString()
                print(value)
            })
